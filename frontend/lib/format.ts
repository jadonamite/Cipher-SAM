...
// Helper function to format the headline and suffixes
function formatHeadlineAndSuffixes(map: CurrencyMap, primary: string): string {
...
export function formatAggregate(map: CurrencyMap): string {
  ...
  return formatHeadlineAndSuffixes(map, primary)
}