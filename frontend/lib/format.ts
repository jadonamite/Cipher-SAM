...
export function formatAggregate(map: CurrencyMap): string {
  const [headline, suffixes] = getHeadlineAndSuffixes(map)
  if (suffixes.length === 0) return headline
